 ...
 */
const getCurrentData = function (ticker: any) {
    return new Promise((resolve, reject) => {
        request(`${baseUrl + ticker}/`, (err: any, res: any, body: any) => {
            if (err) {
                reject(err);
                return;
            }
 ...
 */
const getCurrentPrice = function (ticker: any, callback: any) {
    if (callback) {
        ...
        return getCurrentData(ticker)
            .then((data:any ) => data.price);
    }
};

export { getHistoricalPrices, getCurrentData, getCurrentPrice }